 ...
 * Strongly Connected Components algorithm for the WellingtonTransport project.
 *
 * Creates a small graph (23 stops,26 edges) and then computes the connected components.
 ...
 *
 *  Stops are guaranteed to have unique names.
 ...
 *
 *  The program may number the components in a different order, but the reporting
 *   in this program canonicalises the numbering of the components
 *   so that your results can be compared to the expected output.
 *
 *  Each output line of the test program starts with a | to help distinguish
 *   the test output from any debugging output in the code.
 *
 *  Note, this testing program is not a full test of the program and does not attempt
